...
void WCNSS_SetMacAddress(uint8_t* macAddr) {
    ...
    */
    ...
    if (write(fd, wcnssCommand, 8) < 0) {
        ALOGE("Failed to write to %s : %s", WCNSS_CTRL, strerror(errno));
    }
...
int main(void) {
    ...
    if (efsMacInfoFile = fopen(EFS_MAC_INFO_FILE, "rb"), efsMacInfoFile == NULL) {
        ...
        ALOGE("Failed to open %s: %s, trying default path", EFS_MAC_INFO_FILE, strerror(errno));
        if (efsMacInfoFile = fopen(DEFAULT_EFS_MAC_INFO_FILE, "rb"), efsMacInfoFile == NULL) {
            ALOGE("Failed to open %s: %s", DEFAULT_EFS_MAC_INFO_FILE, strerror(errno));
            return -ENOENT;
        }
    }

    // Get the address
    fread(macAddr, 16, 1, efsMacInfoFile);
    ...
    for (int i = 0; i < sizeof(macAddr); i++) {
        if ((!isdigit(macAddr[i])) && (macAddr[i] != ':') &&
    ...
    sscanf(macAddr, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &formattedMacAddr[0], &formattedMacAddr[1],
           &formattedMacAddr[2], &formattedMacAddr[3], &formattedMacAddr[4], &formattedMacAddr[5]);
